    ...
    public addEvent(eventData: EventInterface) {
        return new Promise((resolve, reject) => {
            ...
            if (event.getSize() > this._batchSize) {
                return reject(new Error(`Event "${eventData.name}" size "${event.getSize()}" is larger than allowed batch size ${this._batchSize}`));
            }
            ...
            if (this._eventsCollection.getSize() >= this._batchSize) {
                this.flushEventCollection();
            } else (this._flushingCollector === null && this._bufferTime !== Infinity) {
